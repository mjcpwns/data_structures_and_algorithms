...
Record DbFindByZid(StudentDb db, int zid) {
    Record dummy = RecordNew(zid, "", "");
    Record r = TreeNext(db->byZid, dummy);
    // Record r = TreeSearch(db->byZid, dummy);
    RecordFree(dummy);
    return r;
}
...
List DbFindByName(StudentDb db, char *familyName, char *givenName) {
    ...
    // Dummy zids are smallest and largest for large range
    Record dummy_low = RecordNew(1, familyName, givenName);
    Record dummy_high = RecordNew(9999999, familyName, givenName);
    List l = TreeSearchBetween(db->byName, dummy_low, dummy_high);

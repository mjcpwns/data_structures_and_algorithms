...
static bool BFSPathCheck(Maze m, Cell start);
static bool setNeighbourState(Maze m, int i, int j, Cell v, Cell **visited, bool **state, Queue q);
static bool validCell(Maze m, Cell c);
static bool checkCell(Cell x, Cell y);

bool solve(Maze m) {
    return BFSPathCheck(m, MazeGetStart(m));
}
...
static bool BFSPathCheck(Maze m, Cell start) {
    ...
    while (!found && QueueSize(q)) {
        v = QueueDequeue(q);
        // set state of all unvisited neighbours to Visited
        // and set v as predecessor cell
        found = setNeighbourState(m, v.row, v.col, v, visited, state, q);
    }
    QueueFree(q);
    if (found) {
        MazeMarkPath(m, v);

...
void IntListInsertInOrder(IntList L, int v)
{
	...
	if (L->first == NULL) { // Empty List
		L->first = L->last = n;
		L->size++;
	} else if (v <= L->first->data) {  // Smallest Value
		struct IntListNode *curr = L->first;
		L->first = n;
		n->next = curr;
		L->size++;
	} else if (v > L->last->data) { // Largest Value
		IntListInsert(L, v);
	} else { // Somewhere in the middle case

		// iterate through the list and insert in sorted list
		struct IntListNode *curr = L->first;
		while (curr != NULL) {
			if (v <= curr->next->data) { // Stop one place before target
				struct IntListNode *temp = curr->next;
				curr->next = n;
				n->next = temp;
				break;
			}
